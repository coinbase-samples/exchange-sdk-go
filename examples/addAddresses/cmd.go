...
func main() {
	...
	if len(os.Args) < 4 {
		log.Fatalf("usage: %s <currency> <address> <label> [destination_tag]", os.Args[0])
	}
	...
	addressSummary := model.AddressSummary{
		Currency: currency,
		To: model.To{
			Address:        address,
	...
	request := &addressbook.AddAddressesRequest{
		Addresses: []model.AddressSummary{addressSummary},
	}

	response, err := addressBookSvc.AddAddresses(context.Background(), request)
	if err != nil {
		log.Fatalf("unable to add addresses: %v", err)
	}
