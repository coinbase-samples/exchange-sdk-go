...
func main() {
	...
	if len(os.Args) < 2 {
		log.Fatalf("usage: %s <conversion_id> [profile_id]", os.Args[0])
	}

	conversionId := os.Args[1]
	...
	response, err := conversionsSvc.GetConversion(context.Background(), request)
	if err != nil {
		log.Fatalf("unable to get conversion: %v", err)
	}
