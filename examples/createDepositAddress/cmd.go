...
func main() {
	...
	if len(os.Args) < 2 {
		log.Fatalf("account ID is required as a command-line argument")
	}
	accountId := os.Args[1]
	...
	response, err := coinbaseAccountsSvc.CreateCryptoAddress(context.Background(), request)
	if err != nil {
		log.Fatalf("unable to create deposit address: %v", err)
	}
